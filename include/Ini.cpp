...
Ini::Ini() {
    m_strIniName = GetExecutableName() + ".ini";
    std::cout << m_strIniName << std::endl;
    ReadConfig(m_strIniName);
}
...
bool Ini::ExtractConfig(const std::string &strLine, std::string &strSection, std::string &strKey, std::string &strValue) {
    // 安全检查
    if (strLine.empty()) {
        return false;
    }
...
bool Ini::ReadConfig(const std::string &strFilename) {
    // 清空设置
    m_settings.clear();

    printf("%s:%d read %s\n", __FILE__, __LINE__, strFilename.c_str());
...
std::string Ini::GetString(const char *szSection, const char *szKey, const char *szDefaultValue) {
...
int Ini::GetInt(const char *szSection, const char *szKey, const int &nDefaultValue) {
    std::string strSection(szSection);
    std::string strKey(szKey);
    std::map<std::string, std::string> mapKeyValue;
    std::map<std::string, std::string>::iterator mapIter;
...
double Ini::GetDouble(const char *szSection, const char *szKey, const double &dDefaultValue) {
    std::string strSection(szSection);
    std::string strKey(szKey);
...
std::string IniGetString(const char *szSection, const char *szKey, const char *szDefaultValue) {
    return Ini::Instance()->GetString(szSection, szKey, szDefaultValue);
}

int IniGetInt(const char *szSection, const char *szKey, const int &nDefaultValue) {
    return Ini::Instance()->GetInt(szSection, szKey, nDefaultValue);
}

double IniGetDouble(const char *szSection, const char *szKey, const double &dDefaultValue) {
    return Ini::Instance()->GetDouble(szSection, szKey, dDefaultValue);
}
